function Challenger(question) {
  ...
  let result = inputs.every((input, index) => {
    ...
    if (JSON.stringify(processed) == JSON.stringify(outputs[index])) {
      ...
      document.querySelectorAll("button")[
        question
      ].innerHTML = `Failed at test ${index + 1}`;
      return false;
    }
  });
  ...
  if (successPerc > 0.2 && successPerc < 0.4) {
    bgimg.style.backgroundImage =
      "URL(https://pbs.twimg.com/media/EuJUu_dUYAE9kRY?format=jpg&name=large)";
  } else if (successPerc > 0.4 && successPerc < 0.6) {
    bgimg.style.backgroundImage =
      "URL(https://pbs.twimg.com/media/EuJUu_fVIAcvvAw?format=jpg&name=large)";
  } else if (successPerc > 0.6 && successPerc < 0.8) {
    bgimg.style.backgroundImage =
      "URL(https://pbs.twimg.com/media/EuJUu_iVkAEQggm?format=jpg&name=large)";
  } else if (successPerc > 0.8) {
    bgimg.style.backgroundImage =
      "URL(https://pbs.twimg.com/media/EuJUO62UYAIynSz?format=jpg&name=large)";
  }
}
...
const renderChallenges = (limit) => {
  ...
  Object.keys(challengeInput).forEach((item, index) => {
    ...
    if (index === 0) {
      ...
      challengeContainer.appendChild(createChallenge(`Assessment ${index}`));
    } else
      challengeContainer.appendChild(
        createChallenge(`Assignment ${index - limit}`)
      );
  });
};
